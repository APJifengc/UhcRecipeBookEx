...
public class JsonItemUtils {
    ...
    public static JsonItemStack getItemFromJson(String jsonString) throws ParseException {
        ...
        if (split.length > 1) {
            if ("mmoitems".equals(split[0])) {
                Validate.isTrue(split.length == 3, "Incomplete parameters - " + type);
                var item = MMOItems.plugin.getItem(split[1].toUpperCase(), split[2].toUpperCase());
                Validate.notNull(item, "Unknown item - " + type);
                return new JsonItemStack(item);
            }
            if ("forge".equals(split[0])) {
                Validate.isTrue(split.length == 2, "Incomplete parameters - " + type);
                var item = FurnaceManager.FurnaceInstance.newItem(Integer.parseInt(split[1].toUpperCase()));
                Validate.notNull(item, "Unknown item - " + type);
                return new JsonItemStack(item);
            }
            if ("golden".equals(split[0])) {
                Validate.isTrue(split.length == 2, "Incomplete parameters - " + type);
                var item = UhcItems.createGoldenHeadItem();
                Validate.notNull(item, "Unknown item - " + type);
                return new JsonItemStack(item);
            }
        }
